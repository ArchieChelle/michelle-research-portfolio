...
### Finding 3: The Zapier education gap — a signal about automation awareness, not just one tool

Zapier ranked last in preference share at **4.0%** — but only **27.6% of respondents had ever heard of it**. That number reframes the result: this isn't low interest, it's low awareness. Users are doing the exact workflows Zapier solves (manual CSV uploads, repetitive cross-platform data entry) without knowing automation is an option.

The implication isn't to deprioritize Zapier — it's to treat education as a prerequisite to adoption. Before users can value automation tools, they need to understand automation as a capability. The strategic move is building that awareness first, with Zapier as one entry point, not the headline.
